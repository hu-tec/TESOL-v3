...
이 문서는 `src` 내에 구현된 웹사이트의 **시각적 룩앤필(Visual Look & Feel)**을 추출하여 정리한 디자인 시스템 가이드라인입니다. 다른 개발자가 새로운 사이트나 페이지를 구축할 때 동일하고 일관된 시각적 경험을 제공할 수 있도록 작성되었습니다. (레이아웃 및 구조 제외)
